...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$db->query_delete("articles","id='".$_REQUEST['id'] ."'");
	$_SESSION['msg']="Your selected article successfully deleted!";
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter article name")){
		return false
	}
}

function del(id,url,performance){
	var a=confirm("Are you sure, you want to delete article: '" + performance +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg">Articles</td>
    ...
    <td align="left" valign="top" class="body_whitebg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      <td align="center" valign="top"><table width="501" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                            ...
                            <tr>
                              <td width="123" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Article Name:</td>
        ...
        <?php
				...
				if(isset($_REQUEST['key']) && trim($_REQUEST['key']) != NULL)
					$query .=" and  article_name LIKE '" .trim($_REQUEST['key']). "%'";
				else if(isset($_REQUEST['enter']) && (int)$_REQUEST['enter']==3)
					$query .=" and  article_name LIKE '" .trim($_REQUEST['cd']). "%'";
				
				
				$sql="select * from articles $query order by $order_by";
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                    ...
                    <tr>
                      <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                      <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>articles/articles-new.php'" /></td>
                      <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                    </tr>
                  </table>
              ...
              <tr>
               	<td width="65%" align="left" valign="middle" bgcolor="#35619c" class="table_heading"><a href="articles/articles.php?display_oder=title&display_oder_type=<?=$display_oder_type?>" class="header_link">Name</a></td>
            	<td width="20%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Date</td>
              	<td width="15%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Action</td>
              </tr>
              <?php if(count($result) == 0){?>
              <tr>
                <td colspan="3" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no articles found</td>
              </tr>
              <?php }else{
					for($j=0;$j<count($result);$j++){?>
              <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                <td  align="left" valign="middle" class="table_content-blue"><?=$result[$j]['article_name']?></td>              
                 <td  align="left" valign="middle" class="table_content-blue"><?=date("dS M, Y",strtotime($result[$j]['article_date']))?></td>           
               
                <td align="center" valign="middle" class="table_content-blue"><img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>articles/articles-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />&nbsp;&nbsp;&nbsp;&nbsp; 
                <img src="images/view-details.png" onclick="location.href='<?=$general_func->admin_url?>articles/articles-view.php?id=<?=$result[$j]['id']?>&action=VIEW&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="VIEW" alt="VIEW" /> &nbsp;&nbsp;&nbsp;&nbsp;             
                <img src="images/delete.png" onclick="del('<?=$result[$j]['id']?>','<?=urlencode($url)?>','<?=$result[$j]['article_name']?>')" style="cursor:pointer;" /> </td>
              </tr>
              <?php }
				}
	  		?>
