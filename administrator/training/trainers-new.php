...
if(isset($_GET['now']) && $_GET['now']=="DELETE"){
	...
	@mysql_query("update trainers set $field=' ' where id=" . (int) $_GET['id'] . "");	
...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){		
	$sql="select * from trainers where id=" . intval($_REQUEST['id'])  . " limit 1";
	...
	$trainer_type=$result[0]['trainer_type']; 	
...
if(isset($_POST['enter']) && $_POST['enter']=="yes" && trim($_POST['login_form_id'])==$_SESSION['login_form_id']){
	...
	$trainer_type=intval($_REQUEST['trainer_type']);		 
	$status=intval($_REQUEST['status']);	
	
	if($_POST['submit']=="Add New"){
		if($db->already_exist_inset("trainers","email_address",$email_address)){
			...
			if($db->already_exist_inset("trainers","seo_link",$data['seo_link'])){//******* exit
				$data['seo_link']=$db->max_id("trainers","id") + 1 ."-".$data['seo_link'];
			}
			...
			$inserted_id=$db->query_insert("trainers",$data);		
			
			
			$refered_code_data['refered_code']=$inserted_id.$general_func->genTicketString(5);;
			$db->query_update("trainers",$refered_code_data,'id='.$inserted_id);
			...
			if($_FILES['photo']['size'] >0 && $general_func->valid_file_type($_FILES["photo"]["name"],$_FILES["photo"]["type"])){			
				...
				$db->query_update("trainers",$uploaded_name,'id='.$inserted_id);
			...
			$_SESSION['msg']="Trainer profile successfully created!";
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}	

	}else{
		if($db->already_exist_update("trainers","id",$_REQUEST['id'],"email_address",$email_address)){
			$_SESSION['msg']="Sorry, your specified email address is already taken!";		
		}else{
			$data['fname']=$fname;	
			$data['lname']=$lname;			
			...
			$data['trainer_type']=$trainer_type;
			$db->query_update("trainers",$data,"id='".$_REQUEST['id'] ."'");
			...
			if($_FILES['photo']['size'] >0 && $general_func->valid_file_type($_FILES["photo"]["name"],$_FILES["photo"]["type"])){
				...
				$uploaded_name['photo']=$path;
				$db->query_update("trainers",$uploaded_name,'id='.$_REQUEST['id']);
			...
			if($db->affected_rows > 0)
				$_SESSION['msg']="Trainer profile successfully updated!";
            ...
            <td width="780" align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="10">
                ...
                <tr>
                  ...
                  <td  valign="top"><?php if(trim($photo) != NULL){?>
                    ...
                    <a href="trainers/trainers-new.php?action=EDIT&now=DELETE&id=<?=$_REQUEST['id']?>&return_url=<?=$return_url?>&field=photo&path=<?=$photo?>" class="htext" ><u>Delete</u></a>
                    <?php }?>
                    <br/>
                    <br/>
                    <?php }	?>
                ...
                <tr>                              
                  <td class="body_content-form" valign="top">Trainer Type:<font class="form_required-field"> *</font></td>
                  <td  valign="top"><select name="trainer_type"  class="inputbox_select" style="width: 150px;" >
                      <option value="1" <?=$trainer_type==1?'selected="selected"':'';?>>Gym</option>
                      <option value="2" <?=$trainer_type==2?'selected="selected"':'';?>>Personal Trainer</option>
                    </select>
                   </td>
                </tr>
                ...
                <tr>                              
                  ...
                  <td  valign="top"><select name="status"  class="inputbox_select" style="width: 150px;" >
