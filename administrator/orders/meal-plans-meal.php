...
$result_meals=$db->fetch_all_array("select id,name from meals where id IN(select DISTINCT(meal_id) from meal_plan_category_meals where meal_plan_category_id='" . intval($result_meal_pan[0]['meal_plan_category_id']) . "')  order by name ASC");
$total_meals=count($result_meals);


$result_snacks=$db->fetch_all_array("select id,name from snacks where id IN(select DISTINCT(snack_id) from meal_plan_category_snacks where meal_plan_category_id='" . intval($result_meal_pan[0]['meal_plan_category_id']) . "' ) order by name ASC");
