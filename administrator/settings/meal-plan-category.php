...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$plan_category_meals_found=$db->num_rows("select id from meal_plan_category_meals where meal_plan_category_id='".$_REQUEST['id'] ."' limit 1");
	$meal_plans_found=$db->num_rows("select id from meal_plans where meal_plan_category_id='".$_REQUEST['id'] ."' limit 1");
	$plan_category_snacks_found=$db->num_rows("select id from meal_plan_category_snacks where meal_plan_category_id='".$_REQUEST['id'] ."' limit 1");
	$orders_found=$db->num_rows("select id from orders where meal_plan_category_id='".$_REQUEST['id'] ."' limit 1");
	
	if($plan_category_meals_found == 0 && $meal_plans_found == 0 && $plan_category_snacks_found == 0 && $orders_found == 0){
		...
		$meassage .= $plan_category_meals_found == 1?' meals and ':'';	
		$meassage .= $plan_category_snacks_found == 1?' snacks and ':'';	
		$meassage .= $orders_found == 1?' orders and ':'';			
...
cellpadding="5" cellspacing="1">
              <tr>
                ...
                <td class="text_numbering" align="right" colspan="4"><?=$total_count?> Categories found.</td>
              </tr>
              <tr>
                     ...
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                        ...
                        <td align="center" valign="middle" class="table_content-blue">                        	
                        	<div class="viewCursor">          
						    	<img src="images/view-details.png" style="cursor: pointer;" />
                              <div class="layoutTxt">
                              <img src="images/pointer.png" alt="" class="pontr" />
                              	  ...
                              	  <p><?php
						            	for($day=1; $day <=7; $day++){
											...
											for($time=1; $time <= intval($result[$j]['meals_per_day']); $time++ ){
												echo "<strong style='padding-top: 5px;'>Meal " . $time .":&nbsp;</strong> " . $default_meals[$day][$time]['meal_category'];
												echo " - ";
