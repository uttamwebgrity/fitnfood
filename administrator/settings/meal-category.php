...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$meal_found=$db->num_rows("select id from meals where status=1 and meal_category_id='".$_REQUEST['id'] ."' limit 1");
	$meal_plan_found=$db->num_rows("select id from meal_plan_layout where meal_category_id='".$_REQUEST['id'] ."' limit 1");
	
	if($meal_found == 0 && $meal_plan_found == 0){
		$db->query_delete("meal_category","id='".$_REQUEST['id'] ."'");
		$_SESSION['msg']="Your selected meal category deleted!";
	}else{
		$meassage="Your selected meal category can not be deleted as";		
		$meassage .= $meal_found == 1?' few meals and':'';		
		$meassage .= $meal_plan_found == 1?' meal plan layout':'';	
		$meassage .= "  are already assigned under this category!";		
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter meal category name")){
		return false
	}
}

function del(id,url,weapon){
	var a=confirm("Are you sure, you want to delete meal category: '" + weapon +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"> Meal Categories</td>
         ...
         <tr>
          <td align="left" valign="middle" height="10"></td>
            ...
            <?php
				...
				$sql="select * from meal_category $query order by $order_by";
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                	...
                	<table border="0"  cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>settings/meal-category-new.php'" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                   </table>
                  
                   </td>
                <td class="text_numbering" align="right" colspan="2"><?=$total_count?> Categories found.</td>
              </tr>
              <tr>
			...
			<?php if(count($result) == 0){?>
                	<tr>
                		<td colspan="3" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no categories added yet!</td>
              		</tr>
				<?php }else{
					for($j=0;$j<count($result);$j++){?>
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                       ...
                       <img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/meal-category-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />
                      &nbsp;&nbsp;&nbsp;&nbsp; 
