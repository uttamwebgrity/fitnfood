...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$snack_found=$db->num_rows("select id from snacks where snacks_type LIKE '%{".$_REQUEST['id'] ."}%' limit 1");
	
	if($snack_found == 0){
		$db->query_delete("snacks_type","id='".$_REQUEST['id'] ."'");
		$_SESSION['msg']="Your selected snack type deleted!";
	}else{
		$meassage="Your selected snack type can not be deleted as ";
		$meassage .= " few snacks are already using it!";		
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter snack type name")){
		return false
	}
}

function del(id,url,weapon){
	var a=confirm("Are you sure, you want to delete snack type: '" + weapon +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"> Snack Types</td>
                        ...
                        <table width="543" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                           <input type="hidden" name="enter" value="3" />
                            ...
                            <tr>
                              <td width="115" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Snack Type:</td>
         ...
         <tr>
          <td align="left" valign="middle" height="10"></td>
            ...
            <?php
				...
				$sql="select * from snacks_type $query order by $order_by";
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                	...
                	<table border="0"  cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>settings/snacks-types-new.php'" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                   </table>
                  
                   </td>
                <td class="text_numbering" align="right" colspan="2"><?=$total_count?> snack type found.</td>
              </tr>
              <tr>
               
                <td width="61%" align="left" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/snacks-types.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Name</a>
                
                </td>
                <td width="19%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/snacks-types.php?display_oder=status&display_oder_type=<?=$display_oder_type?>" class="header_link">Status</a></td>
			...
			<?php if(count($result) == 0){?>
                	<tr>
                		<td colspan="3" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no types added yet!</td>
              		</tr>
				<?php }else{
					for($j=0;$j<count($result);$j++){?>
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                       ...
                       <img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/snacks-types-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />
                      &nbsp;&nbsp;&nbsp;&nbsp; 
