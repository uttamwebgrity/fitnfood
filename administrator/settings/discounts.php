...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$order_found=$db->num_rows("select id from  orders  where program_length='". intval(mysql_real_escape_string($_REQUEST['id'])) ."' limit 1");	
		
	if($order_found == 0){		
		$db->query_delete("discounts","id='". intval($_REQUEST['id']) ."'");	
		$_SESSION['msg']="Your selected discount deleted!";
	}else{
		$_SESSION['msg']="Your selected discount plan can not be deleted as few orders are already using it!";
	}
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter discount name")){
		return false
	}
}

function del(id,url,weapon){
	var a=confirm("Are you sure, you want to delete discount: '" + weapon +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"> Discounts</td>
                        ...
                        <table width="543" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                           <input type="hidden" name="enter" value="3" />
                            ...
                            <tr>
                              <td width="115" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Name:</td>
         ...
         <tr>
          <td align="left" valign="middle" height="10"></td>
            ...
            <?php
				...
				$sql="select * from discounts $query order by $order_by";
              ...
              <tr>
                <td  class="text_numbering">
                	...
                	<table border="0"  cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>settings/discounts-new.php'" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                   </table>
                  
                   </td>
                <td class="text_numbering" align="right" colspan="5"><?=$total_count?> discount found.</td>
              </tr>
              <tr>
               
                <td width="20%" align="left" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/discounts.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Name</a>
                
                </td>
                 <td width="40%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Details</td>
                  <td width="10%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Minimum Order</td>
                <td width="10%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Amount</td>
                
                <td width="10%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/discounts.php?display_oder=status&display_oder_type=<?=$display_oder_type?>" class="header_link">Status</a></td>
                
                <td width="10%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Action</td>
              </tr>
			
			<?php if(count($result) == 0){?>
                	<tr>
                		<td colspan="6" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no discount added yet!</td>
              		</tr>
				<?php }else{
					for($j=0;$j<count($result);$j++){?>
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                        <td  align="left" valign="middle" class="table_content-blue"><?=$result[$j]['name']?></td>
                         <td  align="left" valign="middle" class="table_content-blue"><?=nl2br($result[$j]['details'])?></td>
                         <td  align="center" valign="middle" class="table_content-blue"><?=$result[$j]['minimum_order_weeks'];?> week(s) </td>
                       ...
                       <img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/discounts-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />
                      &nbsp;&nbsp;&nbsp;&nbsp; 
