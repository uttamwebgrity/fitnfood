...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from hear_about_us where id=" .   mysql_real_escape_string(intval($_REQUEST['id'])) . " limit 1";
...
if(isset($_POST['enter']) && $_POST['enter']=="yes" && trim($_POST['login_form_id'])==$_SESSION['login_form_id']){
	...
	if($_POST['submit']=="Add New"){
		if($db->already_exist_inset("hear_about_us","name",$name)){
			$_SESSION['msg']="Sorry, your specified Hear about us name is already taken!";
		}else{
			$data['name']=$name;			
			$data['seo_link']=$general_func->create_seo_link($name);
			
			//*** check whether this name alreay exit ******//
			if($db->already_exist_inset("hear_about_us","seo_link",$data['seo_link'])){//******* exit
				$data['seo_link']=$db->max_id("hear_about_us","id") + 1 ."-".$data['seo_link'];
			}
			//*********************************************//
			...
			$db->query_insert("hear_about_us",$data);
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Hear about us name successfully added!";
				
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}	

	}else{
		if($db->already_exist_update("hear_about_us","id",$_REQUEST['id'],"name",$name)){
			$_SESSION['msg']="Sorry, your specified Hear about us name is already taken!";
		}else{
			$data['name']=$name;			
			$data['seo_link']=$general_func->create_seo_link($name);			
			//*** check whether this name alreay exit ******//
			if($db->already_exist_update("hear_about_us","id",$_REQUEST['id'],"seo_link",$data['seo_link'])){//******* exit
			...
			$db->query_update("hear_about_us",$data,"id='".$_REQUEST['id'] ."'");
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Hear about us name successfully updated!";
...
function validate(){
	if(!validate_text(document.ff.name,1,"Enter hear about us name"))
		return false;
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"><?=$button?> hear about us</td>
        ...
        <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td align="left" valign="top" colspan="3"><table width="79%" border="0"  align="center" cellspacing="2" cellpadding="6">
                <tr>
                  <td width="15%" class="body_content-form" valign="top">Hear About Us:<font class="form_required-field"> *</font></td>
                  ...
                  <td width="85%"><table width="261" border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        ...
                        <td width="49%"><table border="0" align="left" cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="back" onclick="location.href='<?=$general_func->admin_url?>settings/hear-about-us.php'"  type="button" class="submit1" value="Back" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                          </table></td>
                      </tr>
