...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$snack_found=$db->num_rows("select hear_about_us from users where id='". intval(mysql_real_escape_string($_REQUEST['id'])) ."' limit 1");
	
	if($snack_found == 0){
		$db->query_delete("hear_about_us","id='". intval(mysql_real_escape_string($_REQUEST['id'])) ."'");
		$_SESSION['msg']="Your selected hear about us name deleted!";
	}else{
		$meassage="Your selected hear about us name can not be deleted as ";
		$meassage .= " few users are already selected it!";		
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter hear about us name")){
		return false
	}
}

function del(id,url,weapon){
	var a=confirm("Are you sure, you want to delete hear about us name: '" + weapon +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg">Hear About Us</td>
         ...
         <tr>
          <td align="left" valign="middle" height="10"></td>
            ...
            <?php
				...
				$sql="select * from hear_about_us $query order by $order_by";
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                	...
                	<table border="0"  cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>settings/hear-about-us-new.php'" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                   </table>
                  
                   </td>
                <td class="text_numbering" align="right" colspan="2"><?=$total_count?> name found.</td>
              </tr>
              <tr>
               
                <td width="61%" align="left" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/hear-about-us.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Name</a>
                
                </td>
                <td width="19%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/hear-about-us.php?display_oder=status&display_oder_type=<?=$display_oder_type?>" class="header_link">Status</a></td>
			...
			<?php if(count($result) == 0){?>
                	<tr>
                		<td colspan="3" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no name added yet!</td>
              		</tr>
				<?php }else{
					for($j=0;$j<count($result);$j++){?>
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                        <td width="61%" align="left" valign="middle" class="table_content-blue"><?=$result[$j]['name']?></td>
                       <td width="19%" align="center" valign="middle" class="table_content-blue"><?=$result[$j]['status']==1?'Active':'Inactive';?></td>
                       <td width="20%" align="center" valign="middle" class="table_content-blue" >                     
                       <img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/hear-about-us-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />
                      &nbsp;&nbsp;&nbsp;&nbsp; 
