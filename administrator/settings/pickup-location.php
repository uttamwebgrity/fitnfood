...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	$db->query_delete("pickup_locations","id='".$_REQUEST['id'] ."'");	 	
	$_SESSION['msg']="Pickup location successfully deleted!";	
	//*****************************************************************************//	
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter Pickup Location")){
		return false
	}
}

function del(id,url,weapon){
	var a=confirm("Are you sure, you want to delete pickup location: '" + weapon +"'?")
    if (a){
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"> Pickup Location</td>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <table width="543" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                           <input type="hidden" name="enter" value="3" />
                            ...
                            <tr>
                              <td width="115" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Location:</td>
          ...
          <td align="left" valign="middle" height="10"></td>
            ...
            <?php
				...
				if(isset($_REQUEST['key']) && trim($_REQUEST['key']) != NULL)
					$query .=" and  location LIKE '" .trim($_REQUEST['key']). "%'";
				else if(isset($_REQUEST['enter']) && (int)$_REQUEST['enter']==3)
					$query .=" and  location LIKE '" .trim($_REQUEST['cd']). "%'";
				
				
				 $sql="select * from pickup_locations $query order by $order_by";
        ...
        ?> 
        <tr>
          <td align="left" valign="top"><table width="921" align="center" border="0" 
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                	...
                	<table border="0"  cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>settings/pickup-location-new.php'" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                   </table>
                  
                   </td>
                <td class="text_numbering" align="right" colspan="3"><?=$total_count?> Location found.</td>
              </tr>
              <tr>
               
                <td width="30%" align="left" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/pickup-location.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Location</a>
                
                </td>
                 <td width="50%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Street Address</td>               
                
                <td width="10%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/pickup-location.php?display_oder=status&display_oder_type=<?=$display_oder_type?>" class="header_link">Status</a></td>
			...
			<?php if(count($result) == 0){?>
                	<tr>
                		<td colspan="4" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no locations added yet!</td>
              		</tr>
				<?php }else{
					for($j=0;$j<count($result);$j++){?>
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                        <td  align="left" valign="middle" class="table_content-blue"><?=$result[$j]['location']?></td>                       
                        <td  align="left" valign="middle" class="table_content-blue"><?=$result[$j]['address']?></td>
                       ...
                       <img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/pickup-location-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />
                      &nbsp;&nbsp;&nbsp;&nbsp; 
                      <img src="images/delete.png" onclick="del('<?=$result[$j]['id']?>','<?=urlencode($url)?>','<?=$result[$j]['location']?>')" style="cursor:pointer;" />         
