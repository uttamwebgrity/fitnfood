...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$db->query_delete("ask_a_question","id='".$_REQUEST['id'] ."'");
	$_SESSION['msg']="Your selected contact details successfully deleted!";
...
function del(id,url,performance){
	var a=confirm("Are you sure you want to delete contact details of '" + performance +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top" class="body_whitebg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <?php
				...
				$sql="select id,name,email,phone from ask_a_question $query order by $order_by";
        ...
        ?>
        <tr>
          <td align="left" valign="top"><table width="683" align="center" border="0" cellpadding="5" cellspacing="1">
              <tr>                
                <td class="text_numbering" align="right" colspan="4"><?=$total_count?>
                  contacts found.</td>
              </tr>
              <tr>
               	<td width="30%" align="left" valign="middle" bgcolor="#35619c" class="table_heading"><a href="settings/contacts.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Name</a></td>
            	<td width="30%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Email</td>
              	<td width="27%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Phone</td>
      			<td width="13%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">Action</td>
              </tr>
              <?php if(count($result) == 0){?>
              <tr>
                <td colspan="4" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no contacts found</td>
              </tr>
              <?php }else{
              ...
              <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                ...
                <img src="images/view-details.png" onclick="location.href='<?=$general_func->admin_url?>settings/contact-view.php?id=<?=$result[$j]['id']?>&action=VIEW&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="VIEW" alt="VIEW" /> &nbsp;&nbsp;&nbsp;&nbsp;             
                <img src="images/delete.png" onclick="del('<?=$result[$j]['id']?>','<?=urlencode($url)?>','<?=$result[$j]['name']?>')" style="cursor:pointer;" /> </td>
              </tr>
              <?php }
				}
	  		?>
