...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from meal_category where id=" .   intval($_REQUEST['id']) . " limit 1";
...
if(isset($_POST['enter']) && $_POST['enter']=="yes" && trim($_POST['login_form_id'])==$_SESSION['login_form_id']){
	...
	if($_POST['submit']=="Add New"){
		if($db->already_exist_inset("meal_category","name",$name)){
			...
			if($db->already_exist_inset("meal_category","seo_link",$data['seo_link'])){//******* exit
				$data['seo_link']=$db->max_id("meal_category","id") + 1 ."-".$data['seo_link'];
			}
			//*********************************************//
			...
			$db->query_insert("meal_category",$data);
		...
		if($db->already_exist_update("meal_category","id",$_REQUEST['id'],"name",$name)){
			$_SESSION['msg']="Sorry, your specified category is already taken!";
		}else{
			$data['name']=$name;			
			$data['seo_link']=$general_func->create_seo_link($name);			
			...
			$db->query_update("meal_category",$data,"id='".$_REQUEST['id'] ."'");
...
function validate(){
	if(!validate_text(document.ff.name,1,"Enter category name"))
		return false;
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"><?=$button?> Meal Category</td>
        ...
        <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td align="left" valign="top" colspan="3"><table width="79%" border="0"  align="center" cellspacing="2" cellpadding="6">
                <tr>
                  <td width="15%" class="body_content-form" valign="top">Category Name:<font class="form_required-field"> *</font></td>
                  ...
                  <td width="85%"><table width="261" border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        ...
                        <td width="49%"><table border="0" align="left" cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="back" onclick="location.href='<?=$general_func->admin_url?>settings/meal-category.php'"  type="button" class="submit1" value="Back" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                          </table></td>
                      </tr>
