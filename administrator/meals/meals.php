...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$default_meals_found=$db->num_rows("select id from  categories_default_meals  where meal_id='".$_REQUEST['id'] ."' and type=1 limit 1");
	$meal_plans_found=$db->num_rows("select id from  meal_plan_meals where meal_id='".$_REQUEST['id'] ."' and type=1 limit 1");
	$order_meals_found=$db->num_rows("select id from  order_meals where meal_id='".$_REQUEST['id'] ."' and type=1 limit 1");
	
	if($default_meals_found == 0 && $meal_plans_found == 0 && $order_meals_found == 0){
		$sql="select photo_name from meals where id=" . (int) $_REQUEST['id'] . " limit 1";
		...
		$db->query_delete("meals","id='".$_REQUEST['id'] ."'");
	
		$_SESSION['msg']="Your selected meal deleted!";
	}else{
		$meassage="Your selected meal can not be deleted as ";		
		$meassage .= $default_meals_found == 1?' default meal categories and ':'';		
		$meassage .= $meal_plans_found == 1?' meal plans and ':'';	
		$meassage .= $order_meals_found == 1?' some orders and ':'';		
		$meassage = substr($meassage,0, -4);		
		$meassage .= " are already using this meal!";		
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter meal name.")){
		return false;
	}
}

function del(id,url,name){
	var a=confirm("Are you sure, you want to delete meal: '" + name +"'\nAnd all data related to it?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg">Meals</td>
  ...
  <tr>
    <td align="left" valign="top" class="body_whitebg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      <td align="center" valign="top"><table width="501" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                            ...
                            <tr>
                              <td width="123" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Meal Name:</td>
        ...
        <?php
				...
				$sql="select m.id,m.name as meal_name,energy,fat_total,carbs_veggies,with_or_without_sauce,m.status from meals m";
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                    ...
                    <tr>
                      <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                      <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onClick="location.href='<?=$general_func->admin_url?>meals/meals-new.php'" /></td>
                      <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                    </tr>
                 </table>
              ...
              <tr>
                <td width="240"  align="left" valign="middle" class="table_heading">
                <a href="meals/meals.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Meal Name</a></td>
   				<td width="80"  align="center" valign="middle" class="table_heading">Energy</td>
   				<td width="80"  align="center" valign="middle" class="table_heading">Fat Total </td>
   				<td width="120"  align="left" valign="middle" class="table_heading">Carbs and/or Veggies</td>
   				<td width="80"  align="left" valign="middle" class="table_heading">With Sauce?</td>
   				<td width="220"  align="left" valign="middle" class="table_heading">Plan Category</td>
                 <td width="50"  align="center" valign="middle" class="table_heading">
                 	
                 	 <a href="meals/meals.php?display_oder=status&display_oder_type=<?=$display_oder_type?>" class="header_link">Status</a></td>             
                <td width="120"  align="center" valign="middle" class="table_heading">Action</td>
              </tr>
              <?php if(count($result) == 0){?>
              <tr>
                <td colspan="9" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no meals  found!</td>
              </tr>
              <?php }else{
					for($j=0;$j<count($result);$j++){?>
              <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                <td align="left" valign="middle" class="table_content-blue"><?=$result[$j]['meal_name']?></td>
				<td align="center" valign="middle" class="table_content-blue"><?=$result[$j]['energy']?> </td> 
				<td align="center" valign="middle" class="table_content-blue"><?=$result[$j]['fat_total']?></td> 
				<td align="left" valign="middle" class="table_content-blue"><?php
				if($result[$j]['carbs_veggies'] == 1)
					echo "Carbs";
				else if($result[$j]['carbs_veggies'] == 2)
					echo "Veggies";
				else
					echo "Carbs and Veggies";	
				
				?></td> 
				<td align="center" valign="middle" class="table_content-blue"><?=$result[$j]['with_or_without_sauce']==1?'Yes':'No';?></td> 
				<td align="left" valign="middle" class="table_content-blue"><?=$db_common->plan_category($result[$j]['id']);?></td>               
                ...
                <td  align="center" valign="middle" class="table_content-blue">
                	...
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>meals/meals-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" /> 
                	
                	&nbsp;&nbsp; <img src="images/view-details.png" onclick="location.href='<?=$general_func->admin_url?>meals/meals-view.php?id=<?=$result[$j]['id']?>&action=VIEW&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="VIEW" alt="VIEW" />
                	
                	&nbsp;&nbsp; <img src="images/delete.png" title="DELETE" alt="DELETE" onclick="del('<?=$result[$j]['id']?>','<?=urlencode($url)?>','<?=$result[$j]['meal_name']?>')" style="cursor:pointer;" />
