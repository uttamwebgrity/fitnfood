...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$default_meals_found=$db->num_rows("select id from  categories_default_meals  where meal_id='".$_REQUEST['id'] ."' and type=2 limit 1");
	$meal_plans_found=$db->num_rows("select id from  meal_plan_meals where meal_id='".$_REQUEST['id'] ."' and type=2 limit 1");
	$order_meals_found=$db->num_rows("select id from  order_meals where meal_id='".$_REQUEST['id'] ."' and type=2 limit 1");
	...
	if($default_meals_found == 0 && $meal_plans_found == 0 && $order_meals_found == 0){
		...
		$_SESSION['msg']="Your selected snack deleted!";
	}else{
