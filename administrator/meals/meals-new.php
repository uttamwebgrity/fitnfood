...
if(isset($_POST['enter']) && $_POST['enter']=="yes" && trim($_POST['login_form_id'])==$_SESSION['login_form_id']){
	...
	if($_POST['submit']=="Add New"){
		...
		if($db->already_exist_update("meals","id",$_REQUEST['id'],"name",$name)){
			...
			if(intval($status) == 0){
				$default_meals=mysql_num_rows(mysql_query("select meal_id from categories_default_meals where meal_id='".$_REQUEST['id'] ."' and type=1 limit 1"));
				$plan_meals=mysql_num_rows(mysql_query("select meal_id from meal_plan_meals where meal_id='".$_REQUEST['id'] ."' and type=1 limit 1"));
				
				if($default_meals == 1 || $plan_meals == 1){
					$status_canbe_modified=0;
					$status_message =" <br/> but meal status cannot be changed as it is already added to some meal plans";
				}	
			}
			...
			if($db->affected_rows > 0)
				$_SESSION['msg']="Meal successfully updated" .$status_message. ".";
