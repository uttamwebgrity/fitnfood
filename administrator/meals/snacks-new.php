...
if(isset($_POST['enter']) && $_POST['enter']=="yes" && trim($_POST['login_form_id'])==$_SESSION['login_form_id']){
	...
	if($_POST['submit']=="Add New"){
		...
		if($db->already_exist_update("snacks","id",$_REQUEST['id'],"name",$name)){
			...
			$status_canbe_modified=1;
			$default_meals=0;
			$plan_meals=0;
			$status_message="";
			
			if(intval($status) == 0){
				$default_meals=mysql_num_rows(mysql_query("select meal_id from categories_default_meals where meal_id='".$_REQUEST['id'] ."' and type=2 limit 1"));
				$plan_meals=mysql_num_rows(mysql_query("select meal_id from meal_plan_meals where meal_id='".$_REQUEST['id'] ."' and type=2 limit 1"));
				
				if($default_meals == 1 || $plan_meals == 1){
					$status_canbe_modified=0;
					$status_message =" <br/> but snack status cannot be changed as it is already added to some meal plans";
				}	
			}
			
			if(intval($status_canbe_modified) == 1)			
				$data['status']=$status;	
			...
			if($db->affected_rows > 0)
				$_SESSION['msg']="Snack successfully updated" .$status_message. ".";
