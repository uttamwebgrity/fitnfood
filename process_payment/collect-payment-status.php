...
$sql="select p.id as payment_id,o.order_amount,order_id,week_start_date,week_end_date,CONCAT(fname,' ',lname) as user_name,street_address,email_address,notes,order_email_content from payment p";
$sql .=" left join users u on p.user_id=u.id left join orders o on p.order_id=o.id where order_status=0";
...
for($user=0; $user < $total_users; $user++ ){
	...
	if(trim($status_data[1]) != NULL &&  strtolower(trim($status_data[1])) == "successful"){//********** Success		
		...
		//********************************************************************************//		
	}else if(trim($status_data[1]) != NULL &&  strtolower(trim($status_data[1])) == "failed"){//********** Success
...
//$msg = "Cron file successfully executed for gathering debit status: ". date("d/m/Y h:i:s A");
