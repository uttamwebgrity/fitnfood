...
if (isset($_GET['Result']) && strtolower(trim($_GET['Result'])) == "s") {
	...
	if ($order_type == 1) {//******* select your meal plan
		...
		//$db -> query("update orders set meal_plan_category_id='" . intval($_SESSION['choose_your_meal_plan']['meal_plan_category_id']) . "' ,order_email_content='" .stripcslashes($email_content) . "' where id='" . intval($order_id) . "'");
		...
		//$db -> query("update orders set meal_plan_category_id='" . intval($_SESSION['fill_the_questionnaire']['meal_plan_category_id']) . "' ,order_email_content='" .stripcslashes($email_content) . "' where id='" . intval($order_id) . "'");
	...
	$info_URL = "https://www.edebit.com.au/IS/" . trim($accountType) . "Info.aspx?cd_crn=" . $edNo . "-" . $clNo . "&returnURL=" . $returnURL;
	$general_func -> header_redirect($info_URL);
}
...
<div class="paymtInfo">
	<div class="mainDiv2">
		...
		<div style="float:right; border:none;" class="checkout_row">
			<input type="button" value="My Account" name="myaccount" onclick="location.href='<?=$general_func -> site_url?>my-account/'" style="">
		<?php
		if($user_can_download_pdf == 1 && trim($pdf_file_name) != NULL){
