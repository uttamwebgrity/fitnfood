...
unset($_SESSION['payment']['promo_amount']);
unset($_SESSION['payment']['promo_text']);
unset($_SESSION['payment']['how_many_week']);
...
if (isset($_SESSION['user_login_type']) && trim($_SESSION['user_login_type']) == "users") {
	...
	if ($db_common -> user_has_an_active_order(intval($_SESSION['user_id']), 2) > 0) {
		...
		$_SESSION['user_message'] = "Sorry, you can not make another order with us. <br/>
		You need to cancel your previous order before placing a new order. <br/> You can also modify your existing order from your order listing page.";
...
if (isset($_POST['enter']) && trim($_POST['enter']) == "oder_your_meal_plan" && trim($_POST['before_login_form_id']) == $_SESSION['before_login_form_id']) {
	...
	$pickup_delivery = intval($_POST['pickup_delivery']);
	$pickup_location_id = intval($_POST['pickup_location_id']);
	...
	$_SESSION['payment']['pickup_delivery'] = $pickup_delivery;
	$_SESSION['payment']['pickup_location_id'] = $pickup_location_id;
	$_SESSION['payment']['promo_code'] = trim($_POST['promo_code']);
...
<script type="text/javascript">
	...
	function promo_code_checking(){
		var promo_code=$("#promo_code").val().trim();		
		if(promo_code == ""){
			document.getElementById("promo_code").style.border = "1px solid red";	
		}else{
			document.getElementById("promo_code").style.border = "1px solid #DADCDD";
			
			$.get("promo-code-checking.php?promo_code=" + promo_code, function(data) {	
				var promo_code_response = data.split("~_~");
				
				if(parseInt(promo_code_response[0]) == 0){
					 $("#promocode_message").addClass('alert_message2').removeClass('alert_message1');					
				}else{
				 	$("#promocode_message").addClass('alert_message1').removeClass('alert_message2');
				}
						
				$("#promocode_message").html(promo_code_response[1]);
				$("#promocode_message").show(1000);
			});
	...
	function location_timing(location_id) {		
		if(parseInt(location_id) > 0) {
			$.get("pickup-location.php?id=" + location_id + "&query_type=1", function(data) {				
				$("#qoute_location").html(data);
				$("#qoute_location").show(1000);
			});
		} else {			
			$("#qoute_location").html("");
			$("#qoute_location").hide(1000);
		}
	}
	...
	function decide_pickup_location(val) {		
		if(parseInt(val) == 1){
			$("#pickup_location_div").hide('slow');	
			$("#display_delivery_day").show('slow');
		}else{
			$("#pickup_location_div").show('slow');
			$("#display_delivery_day").hide('slow');			
		}
	}		
	...
	function validate_order_your_meal_plan() {
		...
		if(document.frm_order_meal_plan.pickup_delivery[1].checked == true){
			if(document.getElementById("pickup_location_id").value == '') {
				document.getElementById("pickup_location_id").style.border = "1px solid red";
				error++;
			} else {
				document.getElementById("pickup_location_id").style.border = "1px solid #DADCDD";
			}		
		}
																...
																<div class="tip_column_container">
																	<div class="tip_column">
																		...
																		<div class="tip_column_info_row">
																			...
																			<div class="info_tab">
																				<?=$db_common->nutritional_value($default_meals[$day][$time]['energy'],$meal_size)?> kcal
																			</div>
																		</div>
																		<div class="tip_column_info_row">
																			<div class="info_tab">
																			...
																			<div class="info_tab">
																				<?=$db_common->nutritional_value($default_meals[$day][$time]['protein'],$meal_size) ?>g
																			</div>
																		</div>
																		<?php } ?>
																		<div class="tip_column_info_row">
																			...
																			<div class="tip_column">
																				...
																				<div class="tip_column_info_row">
																					...
																					<div class="info_tab">
																						<?=$db_common->nutritional_value($default_meals[$day][$time]['carbohydrates'],$meal_size) ?>g
																					</div>
																				</div>
																				<div class="tip_column_info_row">
																					<div class="info_tab">
																					...
																					<div class="info_tab">
																						<?=$db_common->nutritional_value($default_meals[$day][$time]['fat_total'],$meal_size)?>g
																					</div>
																				</div>
																				<div class="tip_column_info_row">
																					<div class="info_tab">
													...
													<div class="tip_row">
														<div class="tip_column_container">
															<div class="tip_column">
																...
																<div class="tip_column_info_row">
																	<div class="info_tab">Energy :</div>
																	<div class="info_tab"><?=$db_common->nutritional_value($result_default_meals[0]['energy'],$_SESSION['customize_your_meal_plan']['customized_meal'][$day][$time]['meal_size']) ?> kcal</div>
																</div>
																<div class="tip_column_info_row">
																	<div class="info_tab">Protein :</div>
																	<div class="info_tab"><?=$db_common->nutritional_value($result_default_meals[0]['protein'],$_SESSION['customize_your_meal_plan']['customized_meal'][$day][$time]['meal_size']) ?>g</div>
																</div>	
															...
															<div class="tip_column">
																...
																<div class="tip_column_info_row">
																	<div class="info_tab">Calories :</div>
																	<div class="info_tab"><?=$db_common->nutritional_value($result_default_meals[0]['calories'],$_SESSION['customize_your_meal_plan']['customized_meal'][$day][$time]['meal_size'])?>g</div>
																</div>
																
																<div class="tip_column_info_row">
																	<div class="info_tab">Carbs :</div>
																	<div class="info_tab"><?=$db_common->nutritional_value($result_default_meals[0]['carbohydrates'],$_SESSION['customize_your_meal_plan']['customized_meal'][$day][$time]['meal_size']) ?>g</div>
																</div>
																<div class="tip_column_info_row">
																	<div class="info_tab">Total Fat :</div>
																	<div class="info_tab"><?=$db_common->nutritional_value($result_default_meals[0]['fat_total'],$_SESSION['customize_your_meal_plan']['customized_meal'][$day][$time]['meal_size']) ?>g</div>
																</div>
																<div class="tip_column_info_row">
																	<div class="info_tab">Price :</div>
 ...
 ?>
	...
	$total_price +=$general_func->meal_plan_amout_for_training_cost;
						...
						<div class="new_content_row_container">
							<?php
							if(isset($_SESSION['payment']['pickup_delivery'])){
								$pickup_delivery=$_SESSION['payment']['pickup_delivery'];	
							}else{
								$pickup_delivery=1;
							}
							...
							<ul class="choseDlvryPikup">
                            	<li><input onclick="decide_pickup_location('1');" type="radio" <?php echo $pickup_delivery == 1?'checked="checked"':''?>  value="1" name="pickup_delivery" id="r1">
                                <label for="r1"><span></span>Delivery</label></li>
                                <li><input  onclick="decide_pickup_location('2');" type="radio" <?php echo $pickup_delivery == 2?'checked="checked"':''?> value="2" name="pickup_delivery" id="r2">
                                <label for="r2"><span></span>Pickup</label></li>
                            </ul>
                            <div id="pickup_location_div" class="mealPlnColOne normal_select" style="display: <?php echo $pickup_delivery == 2?'block':'none'?>;">
                            <p style="padding-right: 10px; text-align:left;"><strong>Pickup Location</strong></p>
									<label class="custom-select">
										<select name="pickup_location_id" id="pickup_location_id" class="selStlTwo" onchange="location_timing(this.value);">
											<option value="">Choose One</option>
											<?php
											$sql_discount = "select id,location from pickup_locations where status=1 ORDER BY CAST(`location` AS SIGNED) ASC";
											$result_discount = $db -> fetch_all_array($sql_discount);
											$total_discount=count($result_discount);

											for($d=0; $d < $total_discount; $d++){?>
											<option value="<?=$result_discount[$d]['id']?>" <?=(isset($_SESSION['payment']['pickup_location_id']) && $_SESSION['payment']['pickup_location_id'] == $result_discount[$d]['id']) ? 'selected="selected"' : ''; ?> ><?=$result_discount[$d]['location'] ?></option>
											<?php } ?>						        	
										</select></label>
                            <br class="clear" />
                           <p style="padding-right: 10px; text-align:left;" id="qoute_location" style="display: <?php echo isset($_SESSION['payment']['pickup_location_id'])?'block':'none'?>;">
                           	<?php
                           	$rs_pickup_timing= $db->fetch_all_array("select address,pickup_timing from pickup_locations where id='" . intval($_SESSION['payment']['pickup_location_id']). "' limit 1");
							if(count($rs_pickup_timing) == 1){
								echo  "<strong>Address:</strong><br />". trim($rs_pickup_timing[0]['address']). "<br/><br/>";	
								echo "<strong>Pickup Timing:</strong><br />" . nl2br(trim($rs_pickup_timing[0]['pickup_timing']));
							}
                           	?>
                           </p>                           
                            </div>
							<div class="new_content_row">
								<div class="mealPlnColOne normal_select">
									...
									<label class="custom-select">
										<select name="program_length" id="program_length" class="selStlTwo" onchange="discounted_price(this.value);">
											...
											if(isset($_SESSION['customize_your_meal_plan']['meal_per_day']) && intval($_SESSION['customize_your_meal_plan']['meal_per_day']) < 2){
												$query_discount=" and CAST(amt AS SIGNED)=0 "; 	
											}
											
											$sql_discount = "select id,name from discounts where status=1 $query_discount ORDER BY CAST(`name` AS SIGNED) ASC";
											$result_discount = $db -> fetch_all_array($sql_discount);
											$total_discount=count($result_discount);
                               ...
                               <div class="new_content_row">
                                	<div class="form_row">
                                        <label>Promo Code: <strong>(If any)</strong></label>
                                        <input value="<?=$_SESSION['payment']['promo_code']?>" type="text" id="promo_code" name="promo_code">
                                        <input type="button" class="aplyBtn" value="Apply" onclick="promo_code_checking();"  />
                                    </div>
                                    
                                    <div class="form_row" style="padding: 0px;">
										<div class="alert_message1" id="promocode_message" style="display: none; float: left;"></div>
									</div>	
							...
							<div class="update_profile_form">
								...
								<div class="form_row">
									<div class="form_section normal_select">
										...
										<div class="form_section" id="display_delivery_day" style="display: <?php echo $pickup_delivery == 2?'none':'block'?>;">
