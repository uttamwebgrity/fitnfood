...
if (typeof $.ui != 'undefined') {
	$.widget("my.banner_slider_system", {
		...
		_setCookie: function(){
			...
			document.cookie = this.options.cookieName+'='+this.options.cookieVal
					+ "; expires="+this.options.cookieTTL;
		},
...
$(document).ready(function()
{
	if (1 == $('#newsletter-block').length)
	{
		...
		$('.badge-responsive a').append('<div class="ribbon_image"></div">');
});
...
$(document).ready(function() {
	...
	)
	{
		...
		$('.gui-delivery-offer a.gui-offer-add-button').live('click', function() {
			...
			$('#gui-delivery-tabs input[name="offertobuy[' + tid + '][' + oid + '][buy]"]').each(function() {
				$(this).parents('.gui-delivery-offer').addClass('hidden');
			});
			refreshCartInfo();
			return false;
		});
